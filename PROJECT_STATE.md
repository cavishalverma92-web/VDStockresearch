...
- **Render free deployment readiness added (2026-05-07)**:
  - Added `render.yaml` for a Render free Python web service running Streamlit on `$PORT` and binding to `0.0.0.0`.
  - Configured hosted defaults to use yfinance market data, with Kite market data, trading, and portfolio flags disabled.
  - Configured SQLite and generated data directories under `data/` for Render's temporary free-service filesystem.
  - Added `docs/RENDER_DEPLOYMENT.md` with deployment steps, environment policy, free-plan limitations, and Kite secret safety guidance.
  ...
  - Added provider source visibility across Market Today, Data Health refresh output, Data Health source mix, and the default Strategy Scanner results table.
